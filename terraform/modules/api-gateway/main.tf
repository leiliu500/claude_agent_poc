...
# ── Operations dashboard: POST /v1/metrics (durable request log) ──
# A different Lambda from the entrypoint: reading the log needs a VPC attachment to reach RDS, which
# the entrypoint deliberately does not have (it must reach the Bedrock public endpoints). Same token
# authorizer as /v1/ask — the log carries user names and questions, so it is never anonymous.
# Wired only when the database is enabled; without it there is no log to read.
resource "aws_apigatewayv2_integration" "metrics" {
  count                  = var.telemetry_enabled ? 1 : 0
  api_id                 = aws_apigatewayv2_api.this.id
  integration_type       = "AWS_PROXY"
  integration_uri        = var.telemetry_invoke_arn
  integration_method     = "POST"
  payload_format_version = "2.0"
}
